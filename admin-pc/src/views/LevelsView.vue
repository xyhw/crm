  ...
  <div class="page">
    <div class="page-head">
      <div>
        ...
        <p class="page-sub">会员折扣、分佣加成与晋升阈值</p>
      </div>
    ...
    <Modal v-model="editOpen" :title="editItem?.name || editItem?.level_key || '编辑等级'" :dirty="formDirty">
      <label class="field"><span class="field-label">购买折扣(0.9=9折)</span><input v-model="editForm.purchase_discount" class="input" @input="formDirty = true" /><span class="field-help">建议 0.8-0.95，值越小折扣越多</span></label>
      <label class="field"><span class="field-label">分佣加成(0.1=10%)</span><input v-model="editForm.commission_bonus" class="input" @input="formDirty = true" /><span class="field-help">在基础分佣比例上额外加成</span></label>

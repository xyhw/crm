/**
 * 全局配置
 * - H5：同源相对路径，由 nginx / vite devServer 代理转发到后端
 * - 微信小程序：构建期通过环境变量 VITE_API_BASE 注入生产基址
 *   （如 `VITE_API_BASE=https://api.example.com/api npm run build:mp`，
 *   或在 miniapp/.env.production 配置），需同步在小程序后台配置 request 合法域名，
 *   见 miniapp/DEPLOY-CHECKLIST.md；未注入时回落本地联调地址。
 */
const DEV_API_BASE = 'http://127.0.0.1:3001/api';

// #ifdef H5
// 浏览器端走同源相对路径，由 vite devServer 代理转发到后端，避免跨域与 mixed content
export const API_BASE = '/api';
// #endif

// #ifndef H5
export const API_BASE = import.meta.env?.VITE_API_BASE || DEV_API_BASE;
// #endif

export const UPLOAD_BASE = API_BASE.replace(/\/api$/, '');

export const REQUEST_TIMEOUT = 15000;

export const APP_NAME = '商机互助';